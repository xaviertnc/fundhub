...
jQuery(document).ready(function ($) {
  ...
  function renderDirs( dirs, level ) {
    level = level || 0;
    if (level > 3) { return; }
    var html = '';
    for (var dir in dirs ) {
      var url = media_admin_url + searchParam(dir);
      html += '<li class="dir' + (level > 0 ? ' L-' + level : '') + 
        isCurrent(dir) + '">' + '<a href="' + url + '">' + pretty(dir) + 
          '</a></li>';
      if ( ! jQuery.isEmptyObject(dirs[dir]) ) {
        html += renderDirs( dirs[dir], ++level );
      }
    }
